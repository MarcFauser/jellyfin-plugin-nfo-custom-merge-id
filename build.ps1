...
<#
.SYNOPSIS
    Builds Jellyfin.Plugin.NfoCustomMergeId and packages one installable ZIP per Jellyfin line.
...
.DESCRIPTION
    ...
    Structure and reasoning are taken from the sibling plugins in this family, where the
    failure modes guarded against below were measured. Where a comment reports something
    that happened, it says which project it happened in - an inherited justification that
    reads as if it were measured here would be worse than none.
...
param(
    ...
    [string]$RepoName  = 'jellyfin-plugin-nfo-custom-merge-id',

    # The name Jellyfin shows as "Developer", and it is NOT the GitHub account. URLs are the
...
$root       = $PSScriptRoot
$projectDir = Join-Path $root 'Jellyfin.Plugin.NfoCustomMergeId'
$project    = Join-Path $projectDir 'Jellyfin.Plugin.NfoCustomMergeId.csproj'
...
# rewritten manifest.json behind. Found the hard way in the Poster Overlays project - a
# refused run had already replaced the changelog of a published version in the working copy.
if ($Publish)
{
    ...
    $dirty = @(git -C $root status --porcelain -- 'Jellyfin.Plugin.NfoCustomMergeId')
    if ($dirty.Count -gt 0)
    {
        throw "Uncommitted changes under Jellyfin.Plugin.NfoCustomMergeId. Commit them first, or the published ZIP matches no commit:`n  $($dirty -join "`n  ")"
    }
...
# Reproducible artifacts. The project file already suppresses the two channels that would
# otherwise write the HEAD commit into the assembly and the PDB, so the only variable parts
# left are mine - the timestamp written into meta.json and the per-file times
# Compress-Archive stores. Both are pinned to the last commit that touched the plugin
# source, so rebuilding without a source change yields the same ZIP, the same MD5, and a
# published release stays valid. Deliberately not HEAD: committing the manifest or the
# README must not invalidate it.
$stampIso = git -C $root log -1 --format=%cI -- 'Jellyfin.Plugin.NfoCustomMergeId' 2>$null
...
Write-Host "NFO Custom Merge ID  ($pluginId)  timestamp $timestamp" -ForegroundColor Cyan
...
foreach ($t in $targets)
{
    ...
    # The server ships the Jellyfin assemblies itself, so only our own files may travel.
    # Anything else would shadow the server's copy.
    #
    # This plugin should produce nothing foreign in the first place - every Jellyfin
    # reference carries ExcludeAssets="runtime" and StyleCop is PrivateAssets="All". The
    # prune stays as the guarantee rather than the fix, and it is recursive because in the
    # Poster Overlays project a flat version was not enough: SkiaSharp's NATIVE assets are
    # a separate asset group from "runtime" and landed in runtimes\ subdirectories, which a
    # top-level-only prune walked straight past.
    Get-ChildItem -LiteralPath $stageDir -Recurse -File |
        Where-Object { $_.Name -notlike 'Jellyfin.Plugin.NfoCustomMergeId.*' } |
        Remove-Item -Force
    ...
    # The angle brackets in the description are safe: the dashboard renders it as plain JSX
    # text, so React escapes it and <customid> shows up as written rather than being eaten
    # as an unknown tag. Measured on jellyfin-web release-10.11.z, plugin.tsx - the value
    # goes into a <Typography> child, with no dangerouslySetInnerHTML anywhere near it.
    $meta = [ordered]@{
        guid        = $pluginId
        name        = 'NFO Custom Merge ID'
        overview    = 'Makes <customid> readable from a tvshow.nfo, so series grouping is decided on disk.'
        description = 'Jellyfin groups series across release folders by MetadataProvider.Custom, ahead of ' +
                      'TVDB and IMDb, but nothing registers that id - so a <customid> element in a ' +
                      'tvshow.nfo is dropped without a word. This plugin registers it. Two release folders ' +
        ...
        changelog   = ''
        timestamp   = $timestamp
        assemblies  = @('Jellyfin.Plugin.NfoCustomMergeId.dll')
    }
    ...
    $zipName = "jellyfin-plugin-nfo-custom-merge-id_$($t.Version).zip"
...
# (measured in the Poster Overlays project), not as text/plain. Drop a logo.* next to this
# script and it is picked up; without one the field is simply absent, which is valid.
...
# start advertising a checksum that the published release does not have. That happened once in
# the Poster Overlays project: a source change without a version bump, and a plain local build
# quietly rewrote the entry of an already published version. The publish path guards against
# replacing a release; this guards the manifest, which a build without -Publish also touches.
foreach ($t in $targets)
{
...
$fresh = foreach ($t in $targets)
{
    ...
    # reproduced in the Poster Overlays project on an untouched version, same md5, changelog
    # from 323 characters to zero. And unlike a wrong checksum, which aborts the install with
    # an error, an emptied catalogue entry is silent: nobody sees that it is gone.
    #
...
# server first, in the JFLint project. Measured 2026-07-29:
...
# failed for real in a sibling project, and every one of them failed *silently* - the
# plugin simply did not show up, or the install aborted. Hence assertions rather than trust.
Write-Host ""
...
#    belongs on the artifact that ships - in the Poster Overlays project the first build
#    packed 86 MB of native binaries past a prune that only looked at top-level files.
...
foreach ($t in $targets)
{
    ...
    try
    {
        $strays = @($archive.Entries |
            Where-Object { $_.FullName -ne 'meta.json' -and $_.FullName -notlike 'Jellyfin.Plugin.NfoCustomMergeId.*' })
...
# 7. The assembly named in meta.json must actually be in the ZIP. Jellyfin loads exactly
#    what "assemblies" lists; a typo there produces a plugin that installs and then does
#    nothing, which is the failure mode this whole project exists to avoid elsewhere.
foreach ($t in $targets)
{
    $archive = [System.IO.Compression.ZipFile]::OpenRead((Join-Path $distDir $t.ZipName))
    try
    {
        ...
        if (-not $metaEntry)
        {
            throw "$($t.ZipName) has no meta.json - Jellyfin would not load the plugin at all."
        }

        $reader = New-Object System.IO.StreamReader($metaEntry.Open())
        try { $metaBack = ConvertFrom-Json -InputObject $reader.ReadToEnd() } finally { $reader.Dispose() }

        foreach ($asm in @($metaBack.assemblies))
        {
            if ($entryNames -notcontains $asm)
            {
                throw "meta.json in $($t.ZipName) names '$asm', which is not in the archive: $($entryNames -join ', ')"
            }
        }
    }
    finally
...
Write-Host "  ok  every assembly named in meta.json is present in its package"
