...
- **12.0.1.0 links against Jellyfin 12.0.0 final instead of `12.0.0-rc3`.** The published
  ...
  - **And confirmed on a live 12.1.0 server after installing, in both directions**: the
    *Custom Merge ID* field is offered on a **series** and is **absent on a film**. The
    second direction is the one that would have been left out, and without it the first
    only shows that something registered - not that `Supports(item is Series)` actually
    filters. A build proves the contract compiles; only the running server proves the
    plugin loads and its registration is picked up.
  - **Built against 12.0.0 final, running on 12.1.0** - that gap is not an oversight but
    what `targetAbi` is for: the server loads a plugin whose `targetAbi` is at or below its
    own version. Worth keeping apart from the artifact's own number, which is 12.0.1.0.
